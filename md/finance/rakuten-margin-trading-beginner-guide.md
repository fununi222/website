---
title: "楽天証券の信用取引入門：ゼロコースと無期限信用の基本"
date: "2026-08-22"
category: "finance"
author: "FunUni-lab"
description: "楽天証券で信用取引を始める前に、ゼロコース、制度信用6か月と一般信用無期限の違い、保証金・追証、手動注文の確認手順を初心者向けに整理します。"
themes: ["finance:investment", "信用取引", "楽天証券", "資金管理"]
updated: "2026-08-22"
---

# 楽天証券の信用取引入門：ゼロコースと無期限信用の基本
...
- 制度信用（6か月）と一般信用（無期限）の買建をどう比較するか
...
flowchart TD
    ...
    B -->|いいえ| D{6か月を超える保有の可能性を残すか}
    D -->|はい| E[一般信用 無期限<br/>注文画面で取扱を確認]
    D -->|いいえ| F[制度信用 6か月<br/>最終返済日を管理]
    C --> G[余力・維持率・逆指値を確認]
    E --> G
    F --> G
```

この図は選び方の整理であり、無期限を選べば安全という意味ではありません。どの区分でも、株価下落による追証や、証券会社・取引所の規制による制約は受けます。
...
## 3. 制度信用6か月と一般信用（無期限）を比べる

買建に限定すると、判断軸は「期限」「取扱可否」「保有コスト」「自分が期限管理をできるか」です。
...
| 主な管理 | 最終返済日を忘れずに返済する | 長期化した保有の金利・維持率を管理する |

制度信用は6か月の期限がある一方、一般信用（無期限）は返済期日到来による強制決済を気にせず長期保有できます。これは無期限の大きな利点です。 [楽天証券：制度信用](https://www.rakuten-sec.co.jp/web/domestic/margin/system_margin/) [楽天証券：一般信用（無期限）](https://www.rakuten-sec.co.jp/web/domestic/margin/long_term/)

ただし「無期限」は「何も起きない」という意味ではありません。追証、口座の余力不足、取引所規制、株式分割・合併などのコーポレートアクションでは、返済が必要になる場合があります。損切りの判断を先送りする理由にしないことが重要です。

### どちらを選ぶかの考え方

- **制度信用が合いやすいケース**: 数日から数週間程度のスイングを前提にし、返済期日を管理できる場合。6か月という期限を、保有を見直す機会として使えます。
- **一般信用（無期限）が合いやすいケース**: 6か月を超えて保有する可能性を残したい場合、または期日到来での強制決済を避けたい場合。注文画面で買建可能であることを確認したうえで選びます。
- **どちらでも共通すること**: 通常金利は同じなので、信用買いの保有期間が長くなるほど金利負担は増えます。無期限は損失の許容期間を無期限にする仕組みではありません。
...
- 制度信用は6か月の返済期限を管理できる短中期の選択肢です。
- 一般信用（無期限）は期日到来による強制決済を避けられますが、保有コストや追証リスクがなくなるわけではありません。
