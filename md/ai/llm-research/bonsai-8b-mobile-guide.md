...
## 超要約
PrismMLおよびBitNet研究チームが確立した1.58ビット（1-bit）LLM「Bonsai 8B」を、Android (Xiaomi / HyperOS) 端末のTermux環境上で動かし、エッジAIの真髄を体験するためのインタラクティブ・ガイドです。1-bit量子化のメリット（VRAM消費93%削減）の可視化から、Arm NEON最適化、環境構築、ビルド、推論実行までの具体的な手順を解説します。
...
Bonsai 8Bの最大の特徴は、従来の16ビット（FP16）や4ビット（Q4_K_M）と比較した際の圧倒的なメモリ帯域節約効果です。
