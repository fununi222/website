...
<figure class="cyber-glow">`).
  - **Body Content**: Use pure Markdown for all standard text, lists, and headers to minimize maintenance overhead.
- Add frontmatter with `title`, `date`, `category`, `description`, and `themes` whenever possible.
- **Lead Image Convention**:
  - Path: `../assets/img/`
  - Format: `.png`
  - Filename: Match the article's filename (e.g., `my-article.md` uses `../assets/img/my-article.png`).
  - **Structure**: Place the image code directly under `## 超要約`.
